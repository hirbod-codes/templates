...
if [[ -n $san ]]; then
    openssl req -new -config $configFile -noenc -key $key -out $csr -subj /O=my_org/OU=$ou/CN=$cn -addext "subjectAltName = DNS:$san"
else
    openssl req -new -config $configFile -noenc -key $key -out $csr -subj /O=my_org/OU=$ou/CN=$cn
fi
